...
## Match proteins from .pro with FASTA headers
matches <- match(pro, faPro)

## Check for NAs (unmatched proteins)
if (any(is.na(matches))) {
  unmatched_proteins <- pro[is.na(matches)]
  cat("Warning: The following proteins were not found in the FASTA file:\n")
  print(unmatched_proteins)
  
  ## Remove NAs from the matches to avoid errors
  matches <- na.omit(matches)
}

## Subset the FASTA sequences based on matched proteins
fa <- fa[matches]
